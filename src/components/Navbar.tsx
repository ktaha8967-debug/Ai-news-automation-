...
export const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 soft-header">
      {/* Friendly Top News Bar */}
      <div className="bg-[#070a12] border-b border-slate-800/80 text-xs py-2 px-4 text-slate-300">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-300">Articles are verified across multiple official sources</span>
          </div>

          <div className="flex items-center gap-2 text-xs text-slate-300">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
            <span>Average Trust Score: <strong className="text-white font-bold">96.8% Verified</strong></span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-brand-500 via-purple-500 to-emerald-400 p-0.5 shadow-md shadow-brand-500/20 group-hover:scale-105 transition-transform">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="verified-pill text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                100% VERIFIED
              </span>
            </div>
            <p className="text-[11px] text-slate-400 font-medium">Simple, Reliable & Fact-Checked AI News</p>
          </div>
        </Link>

        {/* Clean, Simple & Friendly Navigation */}
        <nav className="hidden lg:flex items-center gap-6 text-sm font-semibold text-slate-300">
          ...
          <Link href="/editorial-standards" className="text-slate-400 hover:text-white transition-colors text-xs border-l border-slate-800 pl-4">
            How We Check Facts
          </Link>
        </nav>
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-[#070a12] border-b border-slate-800 px-4 py-4 space-y-3">
          <Link href="/" onClick={() => setMobileMenuOpen(false)} className="block text-white hover:text-brand-400 py-1 font-bold text-sm">
            Top Stories
          </Link>
          ...
          <div className="pt-2 border-t border-slate-800">
            <Link href="/editorial-standards" onClick={() => setMobileMenuOpen(false)} className="text-slate-400 hover:underline text-xs">
              How We Check Facts
            </Link>
          </div>
        </div>
